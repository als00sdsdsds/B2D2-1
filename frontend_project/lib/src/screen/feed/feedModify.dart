...
class _FeedmodifyState extends State<Feedmodify> {
  ...
  updateFeed() async {
    try {
      Map<String, dynamic> feedResult = await feedConnect.sendFeedUpdate(widget.userInformation['boardNum'], _selectedDate, _contentController.text);
      if(_uploadImages.isNotEmpty || _uploadImages == []) {
        if(feedResult.isNotEmpty || feedResult == '') {
          Get.back();
        }
      }
      Map<String, dynamic> imageResult = await imageConnect.uploadImage(widget.userInformation['boardNum'], _uploadImages);
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Text('글쓴이', style: TextStyle(fontSize: 10.sp),),
                Text(widget.userInformation['boardWriter'],),
              ],
            )
          ),
          // 동아리명
          Container(
            ...
            child: Column(
              ...
              children: [
                Text('글쓴이', style: TextStyle(fontSize: 10.sp),),
                Text(widget.userInformation['boardWriterGroup'],),
              ],
            )
          ),
          // 작성일 입력창
          ...
          Row(
            children: [
              ...
              (_selectedImages == null ||
                        _selectedImages.isEmpty) ? Container() : 
              Container(
                height: 100.w,
                width: 100.w,
                child: ListView.builder(itemCount: _selectedImages.length,
